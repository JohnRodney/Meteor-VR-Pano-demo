...
function applyVector(target, propName, opts, parent) {
  ...
  const refTarget = target[propName];
  ...
  for (const prop in opts.stop) {
    if (typeof(refTarget[prop]) !== 'undefined') {
      parent.amount = (Math.abs(refTarget[prop] - opts.stop[prop])) / (parent.duration / SceneManager.delta);

      if (refTarget[prop] > opts.stop[prop]) {
        refTarget[prop] -= parent.amount;
        changed = true;
      } else if (refTarget[prop] < opts.stop[prop]) {
        refTarget[prop] += parent.amount;
        changed = true;
      }
    }
...
function getParentObject(str, on) {
  ...
  return res.reduce((obj, propName) => {
    return obj[propName];
  }, on);
}
...
function tween(opts, parent) {
  ...
  const target = getParentObject(opts.prop, parent);
  const propName = getPropName(opts.prop);
